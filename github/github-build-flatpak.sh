...
if [ -n "$BUILD_ARCH" ]; then
    ...
    ARCH_ARG="-DCMAKE_SYSTEM_PROCESSOR=${BUILD_ARCH}"
else
    ARCH_ARG=""
fi

if [ "$FLATPAK_BRANCH" = '' ]; then
    cmake ${ARCH_ARG} -DOCPN_TARGET=$OCPN_TARGET -DBUILD_ARCH=$BUILD_ARCH -DOCPN_FLATPAK_CONFIG=ON -DSDK_VER=$SDK_VER -DFLATPAK_BRANCH='beta' $SET_WX_VER ..
else
    cmake ${ARCH_ARG} -DOCPN_TARGET=$OCPN_TARGET -DBUILD_ARCH=$BUILD_ARCH -DOCPN_FLATPAK_CONFIG=ON -DSDK_VER=$SDK_VER -DFLATPAK_BRANCH=$FLATPAK_BRANCH $SET_WX_VER ..
fi
